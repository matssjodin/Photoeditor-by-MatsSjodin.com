...
- **`EditorCanvas.tsx` — the interactive stage.** Composites all visible layers into one display canvas on every `version` change, draws the selection overlay separately (plus shape/gradient previews, clone-source marker and free-transform handles), and handles all pointer/keyboard/wheel input (pan, zoom, paint, select, move/scale/rotate, lasso, shape, gradient, clone, in-canvas text editing). Tools dispatch off `tool.tool`. Wheel zoom uses a **native non-passive listener** (React's synthetic `onWheel` is passive, so `preventDefault()` there is a no-op). Free-transform math lives in `transform.ts` (anchor-fixed `scaleDrag`/`rotateDrag`).

- **`draw.ts` — doc-space drawing + pixel ops.** Shape/gradient geometry, `clippedLayerDraw` (runs a doc-space draw against a layer canvas, compensating the layer's x/y offset and clipping to rect/mask selections, with optional composite op and mask target), and the pixel tools built on the same convention: `paintStamp` (brush/eraser, also mask editing), `cloneStamp`, `floodFill`. **All pixel edits take document coordinates and compensate for the layer offset; rotation/scale are intentionally ignored.** Never write a pixel op that draws doc coords straight onto a layer canvas.
...
- **UI shell:** `Editor.tsx` (layout + drag-drop file open + paste + autosave/restore + the sonner `Toaster`), `TopBar.tsx` (new/open/save project/export/undo), `Toolbar.tsx` (tool selection), `RightPanels.tsx` (layers incl. masks & background removal, adjustments, filters, tool options), `open-file.ts` (shared open flow for Open/drag-drop with toast errors), `fonts.ts` (text font list).

- **Crop and resize semantics.** `cropToSelection` never re-renders pixels — the document shrinks and every layer shifts by the selection origin, so off-canvas pixels survive and rotated/scaled layers stay intact. `resizeDocument` scales each raster canvas by its own size (layers may be smaller/larger than the doc) and scales every layer's x/y.
